/*
  ...
  2019/04  tgames small changes to fix issue with sound extraction
...
#ifdef LZCHECKCRC
	unsigned char crc = 0;
	for (size_t i = 0; i < sizeof(LZHEADER); i++)
		crc += inp[i];
...
int ProcessPalette(unsigned char* input, RGB* palet)
{
	...
	while (*(unsigned short*)input != 0xFFFF)
	{
		...
		for (size_t i = 0; i < cnt * sizeof(palet[0]); i++)
		{
			pal[i] = *input++;
		}
	}
...
void test(char* fn)
{
	...
	//printf("FLAGS %X %X\n", ~0x1FF & *(unsigned short*)&chk[0], ~0xFF & *(unsigned short*)&chk[2]);
						...
						if (h != 0)
						{
							...
							//							for(size_t i = 0;i < 320 * 200;i++) buff[i] = back[i];
							RenderOver(temp, buff, x, y, w, h, mode, 320, 200);
						}
						char tgan[32];
						sprintf(tgan, "frame_%04d.tga", frmno++);
						SaveTGA(tgan, &TGAHeader, 320, 200, dacpal, buff);
						break;
					}
				}
			}
...
int main(int argc, char** argv)
{
	if (argc > 1)
		...
		printf("Usage: hnmExtractor inputfile.hnm\n");
	return 0;
}